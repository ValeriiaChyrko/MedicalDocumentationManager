...
public class RequestAddressDtoValidator : AbstractValidator<RequestAddressDto>
{
    ...
    public RequestAddressDtoValidator()
    {
        RuleFor(x => x.Street)
            ...
            .MaximumLength(MaxLengthPropertyLength)
            .WithMessage($"Street must not exceed {MaxLengthPropertyLength} characters.");

        RuleFor(x => x.City)
            .NotEmpty().WithMessage("City is required.")
            .MaximumLength(MaxLengthPropertyLength)
            .WithMessage($"City must not exceed {MaxLengthPropertyLength} characters.");

        RuleFor(x => x.State)
            .NotEmpty().WithMessage("State is required.")
            .MaximumLength(MaxLengthPropertyLength)
            .WithMessage($"State must not exceed {MaxLengthPropertyLength} characters.");
